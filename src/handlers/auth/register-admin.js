...
const registerAdminSchema = Joi.object({
  ...
  masterKey: Joi.string().required().messages({
    'any.required': 'Se requiere clave maestra para crear administradores'
  })
});
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Verificar master key (clave secreta para crear admins)
    if (value.masterKey !== process.env.ADMIN_MASTER_KEY) {
      return {
        statusCode: 403,
        headers,
        body: JSON.stringify({
          success: false,
          message: 'Clave maestra inválida'
        })
      };
    }
    ...
    const [existing] = await connection.execute(
      'SELECT idAdministrador FROM Administrador WHERE email = ? OR nombreUsuario = ?',
    ...
    const [result] = await connection.execute(
      'INSERT INTO Administrador (nombreUsuario, email, masterPassword, fechaRegistro) VALUES (?, ?, ?, NOW())',
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        idAdministrador: result.insertId
      })
    };
